            ...
            for r in self.rocks:
                ...
                if dist_to > r.radius:
                    ...
                    if lead_dist > 1e-6:
                        ...
                        aim_r = max(0.0, approach) * 2.5 + max(0.0, fire_dot) * 0.3
...
def train(num_episodes=50000, save_every=100, model_path="dqn_model.pt", clear_buffer=False):
    ...
    #   Wave 1 (1 big → 7 kills × 8 = 56 + 8 wave bonus + ~3 center + ~6 survival): ~73
    #   Wave 2 (2 bigs → 14 kills × 8 = 112 + 16 wave bonus + ~3 + ~9 survival):   ~140
    #   After 2 waves (~2000 steps remaining × 0.01):                                 ~20
    #   2-wave total: ~233.   Dodge-only agent (3500 × 0.01): ~35.
    # Threshold 220 requires solid 2-wave play. A non-clearing agent scores ~35.
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # avg >= 220: requires solid 2-wave play; level 2 pressure drives toward 3-wave.
        # avglen >= 600: agent must survive long enough to attempt multi-wave play.
        # avg_waves >= 2.0: rolling mean of waves cleared per episode must be ≥2.0.
